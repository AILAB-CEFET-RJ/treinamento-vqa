...
TRIPLES_FILE = os.path.join(DATA_DIR, "vqa_train", "triples_4.csv") 
...
logging.basicConfig(level=logging.DEBUG,
                    ...
                    datefmt='%m-%d %H:%M',
                    filename= os.path.join(DATA_DIR, "logs", "pre_process.log"),
                    filemode='w')
...
best_model_name = get_model_file(VQA_DIR, "mobilenet-destilation-3", "dot", "best")
logger.debug("model name : %s", best_model_name)

csv_logger = CSVLogger(os.path.join("..","logs", 'preprocess_epochs_destilation_3.csv'))
checkpoint = ModelCheckpoint(best_model_name, save_best_only=True)

logger.debug("gravando dados das epocas em %s", os.path.join("..","logs", 'preprocess_epochs_destilation_3.csv'))
...
plt.savefig("../graphs/best_mobilenet_destilation_3.png")
plt.close()

final_model_name = get_model_file(VQA_DIR, "mobilenet-destilation3", "dot", "final")
