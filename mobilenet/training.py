...
pair_gen = pair_generator(triples_data, image_cache, datagens, 128)
[X1, X2], Y = pair_gen.__next__()

vecs = [np.random.random((10,)), np.random.random((10,))]
s = cosine_distance(vecs)

mobilenet_1 = mobilenet.MobileNet(weights="imagenet", include_top=True)
mobilenet_2 = mobilenet.MobileNet(weights="imagenet", include_top=True)
...
for layer in mobilenet_1.layers:
    layer.trainable = False
    layer.name = layer.name + "_1"
for layer in mobilenet_2.layers:
    layer.trainable = False
    layer.name = layer.name + "_2"

# as saidas da rede serao conecatadas ao ultimo pedaco da rede
vector_1 = mobilenet_1.get_layer("global_average_pooling2d_1_1").output
vector_2 = mobilenet_2.get_layer("global_average_pooling2d_2_2").output

# carregando a rede pre-treinada
siamese_head = load_model(os.path.join(VQA_DIR, "models", BEST_MODEL_FILENAME))
...
model = Model(inputs=[mobilenet_1.input, mobilenet_2.input], outputs=predicao)
...
plt.tight_layout()
plt.savefig("../graphs/best-mobilenet-1.png")
plt.close()
