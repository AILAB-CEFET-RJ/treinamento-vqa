...
logging.basicConfig(level=logging.INFO,
                    ...
                    datefmt='%m-%d %H:%M',
                    filename='../logs/batch_predict_mobilenet.log',
                    filemode='w')
...
logger.debug("IMAGENET SYNSETS %s", len(synsets))
logger.debug("VQA_IMAGES %s", len(vqa_filenames_list))
...
for filename in vqa_filenames_list:    
    ...
    for synset in synsets:
        ...
        logger.debug("Predizendo similaridades...")        
        predicoes = model.predict_generator(pair_generator(pairs_data, image_cache, None, BATCH_SIZE), verbose=0, steps=STEPS, use_multiprocessing=False)
        ...
        logger.debug("Salvando as predicoes...")
